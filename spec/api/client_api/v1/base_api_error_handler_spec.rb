...
RSpec.describe ClientAPI::V1::BaseAPI, type: :api do
  ...
  describe 'ActiveRecord::RecordInvalid' do
    before do
      api.get '/example' do
        webhook = Integrations::Webhook.new
        webhook.errors.add(:user_id, 'some error')

        raise ActiveRecord::RecordInvalid, webhook
      end
    end
  ...
  describe 'ActiveRecord::RecordNotDestroyed' do
    before do
      api.get '/example' do
        webhook = Integrations::Webhook.new
        webhook.errors.add(:base, 'not destroyed error')

        raise ActiveRecord::RecordNotDestroyed.new('message', webhook)
      end
    end
